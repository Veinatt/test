...
for (let i = 0; i < 120; i++) {
  ...
  if (i < 9) {
    img.src = `img/panel_render/panel000${i + 1}.jpg`;
  } else if (i < 99) {
    img.src = `img/panel_render/panel00${i + 1}.jpg`;
  } else {
    img.src = `img/panel_render/panel0${i + 1}.jpg`;
  }
  img.classList.add('block-image');
  img.style.display = 'none';
  images.push(img);
  document.querySelector('.zlk-main-sec-6 .ooo .cont').appendChild(img);
...
var tween_1 = TweenMax.to(obj, 0.5, {
  ...
  onUpdate: function () {
    ...
    images[obj.curImg].style.display = '';
  }
});
// build scene
