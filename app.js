...
for (let i = 0; i < 120; i++) {
  let img;
  if (i < 9) {
    img = `img/POGRUZKA/POGRUZKA000${i + 1}.jpg`;
  } else if (i < 99) {
    img = `img/POGRUZKA/POGRUZKA00${i + 1}.jpg`;
  } else {
    img = `img/POGRUZKA/POGRUZKA0${i + 1}.jpg`;
  }
...
for (let i = 0; i < 300; i++) {
  let img;
  if (i < 9) {
    img = `img/SBORKA_KONTEINERA/SBORKA000${i + 1}.jpg`;
  } else if (i < 99) {
    img = `img/SBORKA_KONTEINERA/SBORKA00${i + 1}.jpg`;
  } else {
    img = `img/SBORKA_KONTEINERA/SBORKA0${i + 1}.jpg`;
  }
  images_main_3.push(img);
}

// create tween
var tween_3 = TweenMax.to(obj, 0.5, {
  curImg: images_main_3.length - 1, // animate propery curImg to number of images
  roundProps: "curImg", // only integers so it can be used as an array index
  repeat: 3, // repeat 3 times
  immediateRender: true, // load first image automatically
  ease: Linear.easeNone, // show every image the same ammount of time
  onUpdate: function () {
    $("#myimg_3").attr("src", images_main_3[obj.curImg]);
  }
});

// build scene
var scene_3 = new ScrollMagic.Scene({
    triggerElement: ".zlk-main-sec-9",
    triggerHook: 0,
    duration: 3000
  })
  .setTween(tween_3) // add indicators (requires plugin)
  .addTo(controller)
...
for (let i = 0; i < 250; i++) {
  let img;
  if (i < 9) {
    img = `img/postroyka_doma/postroyka000${i + 1}.jpg`;
  } else if (i < 99) {
    img = `img/postroyka_doma/postroyka00${i + 1}.jpg`;
  } else {
    img = `img/postroyka_doma/postroyka0${i + 1}.jpg`;
  }
